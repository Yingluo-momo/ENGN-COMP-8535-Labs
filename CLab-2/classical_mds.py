...
def compute_distance_matrix(gaussian_noise):
  ...
  if gaussian_noise:
    noise = np.random.normal(len(australian_cities))
  # cities_mean = np.mean(australian_cities, axis = 0)
  # normed_cities = australian_cities - cities_mean
  # distance_matrix = normed_cities.T@normed_cities
...
def classical_mds(distance_matrix, k):
  ...
  #get the eigen vectors and eigen values of B
